...
`a365 setup all` supports two distinct agent types, controlled by `--ownidentity` (CLI) or `aiTeammate` (config):

| Agent Type | Flag | What it creates |
|---|---|---|
| **Own-identity agent** (default) | `--ownidentity true` or omit | Azure infra + Agent Blueprint + batch permissions (5 resources) + messaging endpoint |
| **Custom Engine Agent / Blueprint** | `--ownidentity false` | Agent Blueprint + batch permissions (Graph + A365 Tools only) + Agent Instance (Graph API) |
