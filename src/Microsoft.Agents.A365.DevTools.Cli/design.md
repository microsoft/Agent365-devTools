...
|---|---|---|
| **Own-identity agent** | `--aiteammate true` | Azure infra + Agent Blueprint + batch permissions (5 resources) + messaging endpoint |
| **Custom Engine Agent / Blueprint** (default) | `--aiteammate false` or omit | Agent Blueprint + batch permissions (Graph + A365 Tools only) + Agent Instance (Graph API) |
