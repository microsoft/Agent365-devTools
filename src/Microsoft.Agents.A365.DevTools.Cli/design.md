...
|---|---|---|
| **Own-identity agent** | `--ownaccess` | Azure infra + Agent Blueprint + batch permissions (5 resources) + messaging endpoint |
| **Custom Engine Agent / Blueprint** (default) | omit `--ownaccess` | Agent Blueprint + batch permissions (Graph + A365 Tools only) + Agent Instance (Graph API) |
