...
public class AuthenticationServiceTests : IDisposable
{
    ...
    [Fact]
    public async Task GetAccessTokenAsync_WhenBrowserBlockedByConditionalAccessPolicy_FallsBackToDeviceCode()
    {
        // Arrange — browser credential blocked by Conditional Access Policy (AADSTS53003)
        var msalServiceEx = new Microsoft.Identity.Client.MsalServiceException(
            "access_denied",
            $"AADSTS53003: Access has been blocked by Conditional Access policies. Contact your IT administrator.");
        var browserCredential = new ThrowingTokenCredential(
            new MsalAuthenticationFailedException(
                $"Interactive authentication blocked by Conditional Access Policy ({AuthenticationConstants.ConditionalAccessPolicyBlockedError}).",
                msalServiceEx));

        var expectedToken = "device-code-cap-fallback-token";
        var deviceCodeCredential = new StubTokenCredential(expectedToken, DateTimeOffset.UtcNow.AddHours(1));
        ...
        try
        {
            ...
            // Assert
            result.Should().Be(expectedToken,
                because: "when Conditional Access Policy blocks browser auth (AADSTS53003), " +
                         "the CLI must automatically fall back to device code flow so the user is not blocked");
        }
        finally
        {
            sut.ClearCache();
    ...
    [Fact]
    public async Task GetAccessTokenAsync_WhenBrowserBlockedByDeviceCompliancePolicy_FallsBackToDeviceCode()
    {
        // Arrange — browser credential blocked by device compliance policy (AADSTS53000)
        var msalServiceEx = new Microsoft.Identity.Client.MsalServiceException(
            "access_denied",
            $"AADSTS53000: Device is not in a required state.");
        var browserCredential = new ThrowingTokenCredential(
            new MsalAuthenticationFailedException(
                $"Interactive authentication blocked by device compliance policy ({AuthenticationConstants.DeviceCompliancePolicyBlockedError}).",
                msalServiceEx));

        var expectedToken = "device-code-compliance-fallback-token";
        var deviceCodeCredential = new StubTokenCredential(expectedToken, DateTimeOffset.UtcNow.AddHours(1));
        ...
        try
        {
            ...
            // Assert
            result.Should().Be(expectedToken,
                because: "when device compliance policy blocks browser auth (AADSTS53000), " +
                         "the CLI must automatically fall back to device code flow so the user is not blocked");
        }
        finally
        {
    ...
    [Fact]
    public async Task GetAccessTokenAsync_WhenBrowserBlockedByConditionalAccessPolicy_LogsWarning()
    {
        // Arrange
        var msalServiceEx = new Microsoft.Identity.Client.MsalServiceException(
            "access_denied",
        ...
        try
        {
            ...
            // Assert — warning logged explaining why device code was triggered
            logger.Received().Log(
                LogLevel.Warning,
                Arg.Any<EventId>(),
                Arg.Is<object>(o => o.ToString()!.Contains("Conditional Access Policy")),
                Arg.Any<Exception?>(),
                Arg.Any<Func<object, Exception?, string>>());
        }
        finally
