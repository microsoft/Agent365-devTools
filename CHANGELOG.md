...
- `--device-code` flag on `a365 develop get-token` — forces device code authentication instead of browser/WAM. Use it when browser/WAM authentication is rejected for the requested Microsoft Graph scopes — most commonly Exchange-specific scopes such as `MailboxSettings.ReadWrite` and `ExchangeMessageTrace.Read.All`, where the WAM broker on Windows returns `ApiContractViolation` ("declined scopes are present"). Opens https://microsoft.com/devicelogin instead of a WAM popup. Device code does not use the broker.

### Fixed
- Browser/WAM authentication that fails with `ApiContractViolation` ("declined scopes are present") now automatically falls back to device code flow. Previously the error was rethrown as an opaque MSAL failure with no token. Most commonly seen when requesting Exchange-specific Microsoft Graph delegated scopes through the WAM broker on Windows; device code does not use the broker and completes. The fallback is distinct from the admin-consent-required path (`0xcaa90019`), which still exits with consent guidance rather than retrying.
