...
### Added
- `logs export [command] [--output <dir>]` — exports a redacted copy of a CLI diagnostic log safe to share with Microsoft support. Redacts JWT tokens, email addresses, and GUIDs; replaces identical values with consistent aliases so log correlation is preserved. Omit `[command]` to export all available logs at once.
