...
### Added
- `--m365` opt-in flag on `a365 setup blueprint`, `a365 cleanup blueprint`, and `a365 setup all` — when set, the CLI registers or clears the agent blueprint's messaging endpoint via the Teams Graph backend configuration endpoint on MCP Platform. Default is **off**: without `--m365`, endpoint registration is skipped and the CLI points users at the Teams Developer Portal (https://learn.microsoft.com/en-us/microsoft-agent-365/developer/create-instance#1-configure-agent-in-teams-developer-portal) for manual configuration. Intended for M365 agents; the flag is opt-in because the Teams Graph rollout on MCP Platform completes 2026-05-01.
- Messaging endpoint row added to `a365 setup all` summary output, with "registered"/"reused"/"skipped (non-M365)"/"manual config required"/"failed" states. When the server is still on the pre-migration contract, the summary logs a warning and surfaces an "Action Required" entry with the Teams Developer Portal URL so the user knows exactly what to do next.
- Graceful fallback when the server is still on the pre-migration contract — the CLI logs an informational message ("Automated messaging endpoint registration is not available for this tenant yet") rather than a failure, and directs the user to the Teams Developer Portal. After 2026-05-01 this escalates to a warning with a diagnostic hint.
