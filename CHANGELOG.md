...
- `setup all` now retries agent identity creation and blueprint token acquisition with exponential back-off (up to ~60 s total) when Entra replication lag causes transient 401/AADSTS errors on fresh blueprint setups. Retry progress is logged at `Debug` level only.
...
- `setup all` summary now shows **Messaging endpoint — not configured** (with instructions to add `messagingEndpoint` to `a365.config.json`) instead of **failed — see Action Required** when the messaging endpoint URL is absent from config. Real failures (bad URL, contract mismatch) continue to show the error path.
