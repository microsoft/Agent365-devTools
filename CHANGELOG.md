...
### Fixed
- `setup blueprint` now accepts `--service-tree-id`, so blueprint creation succeeds in tenants that require a ServiceTree reference on application registration instead of failing with `ServiceTreeValueMissing`.
