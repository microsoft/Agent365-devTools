...
- `--device-code` option on `a365 develop-mcp grant-agent-mcpserver-permissions` — signs in with a device code instead of the browser or Windows sign-in dialog, for embedded and remote terminals.
- `a365 develop-mcp grant-agent-mcpserver-permissions --agent-blueprint-id <GUID> --mcp-server-name <NAME>` reports which agent instances of a blueprint are missing the permission to call a BYO MCP server, and prompts you to select which ones to grant it to.
