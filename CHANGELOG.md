...
### Changed
- `a365 setup all` / `setup permissions` now prompts for confirmation before creating tenant-wide (AllPrincipals) OAuth2 permission grants when the signed-in user is a Global Administrator running interactively. The prompt shows the blueprint name, app ID, and the exact scopes that will be granted. Unattended callers that do not supply a confirmation provider (e.g. scripts using the SDK directly) continue to grant silently as before.
